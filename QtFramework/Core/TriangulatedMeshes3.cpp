...
GLboolean TriangulatedMesh3::SaveToOFF(const std::string& file_name) const
{
    ...
    if (!f.good())
        return GL_FALSE;
    f<<"OFF" << "\n";;
    f<<_vertex.size()<< " " << _face.size()<< "\n";
    for (std::vector<DCoordinate3>::const_iterator it = _vertex.begin(); it!=_vertex.end();++it)
        f << *it << "\n";
    for (std::vector<DCoordinate3>::const_iterator it = _normal.begin(); it!=_normal.end();++it)
        f << *it << "\n";
    for (std::vector<TCoordinate4>::const_iterator it = _tex.begin(); it!=_tex.end();++it)
        f << *it << "\n";
    for (std::vector<TriangularFace>::const_iterator it = _face.begin(); it!=_face.end();++it)
        f << *it << "\n";
    f.close();
    return GL_FALSE;
}
